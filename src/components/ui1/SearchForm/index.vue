                        ...
                        <el-form-item v-for="item in formItems" :key="item.prop" :prop="item.prop"
                            ...
                            <template #label>
                                <el-text line-clamp="1"
                                    ...
                                    v-if="item.label">
                                    {{isUseLang ? $t(item.label) : item.label}} <span v-if="item.type === 'currency'">(<SvgIcon name="currency" style="width: 12px;color: #1a1a1a;"></SvgIcon>)</span>
                                </el-text>
                            </template>
                            <RangeNumber :config="{item,form}" v-if="item.compType === 'RangeNumber'" />
                            <DynamicFormComp 
                                v-else
